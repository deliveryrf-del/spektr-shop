...
### Проверка /admin входа/выхода
1. Задайте `ADMIN_EMAIL` и `ADMIN_PASSWORD` в `.env`.
2. Запустите `npm run dev` и откройте http://localhost:3000/admin.
3. Введите данные из `.env` и отправьте форму.
4. После входа появятся ссылки «Категории», «Товары» и кнопка «Выйти».
5. Нажмите «Выйти» для выхода.
